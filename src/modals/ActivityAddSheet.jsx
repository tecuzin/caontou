...
const COLORS = ['#5b7042', '#cf7d3c', '#4f8a86', '#9c6b4a', '#8a8b3d', '#b8503f']

/** Feuille « Ajouter activité » au planning — horaire, titre, couleur. */
export function ActivityAddSheet({ sx, onClose, time, setTime, title, setTitle, color, setColor, onSubmit }) {
  return (
    <ModalShell onClose={onClose} z={200}>
      <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()} style={sx('background:#f6efe2;border-radius:28px 28px 0 0;padding:18px 18px 30px;animation:sheetUp 0.3s cubic-bezier(0.2,0.8,0.2,1);')}>
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex : Visite musee" style={sx('width:100%;margin-top:6px;margin-bottom:14px;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')} />
        ...
        <div style={sx('display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;margin-bottom:20px;')}>
          {COLORS.map((c) => (
            <button key={c} onClick={() => setColor(c)} style={sx(`width:32px;height:32px;border-radius:50%;background:${c};border:${color === c ? '3px solid #2f2a22' : '2px solid #d8cbb0'};cursor:pointer;`)} />
          ))}
        </div>
        <div style={sx('display:flex;gap:10px;')}>
          ...
          <button onClick={onSubmit} style={sx('flex:1;border:none;background:#4a5d3a;color:#fffaf0;font-weight:700;font-family:Quicksand;font-size:15px;border-radius:14px;padding:12px;cursor:pointer;')}>Ajouter</button>
        </div>
      </div>
    </ModalShell>
  )

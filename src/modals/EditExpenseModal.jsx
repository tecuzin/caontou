    ...
    <ModalShell onClose={onClose} z={200} fade={false}>
      <div role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={sx('width:100%;background:#f6efe2;border-radius:28px 28px 0 0;padding:20px;max-height:80vh;overflow-y:auto;animation:sheetUp 0.3s;')}>
        ...
        <div style={sx('margin-bottom:12px;')}>
          ...
          <input id="edit-exp-label" type="text" value={editLabel} onChange={e => setEditLabel(e.target.value)} style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')} />
        </div>
        <div style={sx('margin-bottom:12px;')}>
          ...
          <select id="edit-exp-cat" value={editCat} onChange={e => setEditCat(e.target.value)} style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')}>
        ...
        <div style={sx('margin-bottom:14px;')}>
          ...
          <input id="edit-exp-amt" type="number" step="0.01" value={editAmt} onChange={e => setEditAmt(e.target.value)} style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')} />
        </div>
        <div style={sx('display:flex;gap:10px;')}>
          ...
          <button onClick={onDelete} style={sx('flex:1;border:1px solid #b8503f;background:#f7e2dc;color:#b8503f;font-weight:600;border-radius:14px;padding:12px;cursor:pointer;')}>🗑️ Supprimer</button>
        </div>
      </div>
    </ModalShell>
  )

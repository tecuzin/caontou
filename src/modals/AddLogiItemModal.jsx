  ...
  return (
    <ModalShell onClose={onClose} z={200} fade={false}>
      <div role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={sx('width:100%;background:#f6efe2;border-radius:28px 28px 0 0;padding:20px;max-height:80vh;overflow-y:auto;animation:sheetUp 0.3s;')}>
        ...
        <input ref={inputRef} type="text" value={newLogiItem} onChange={e => setNewLogiItem(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && onSubmitAndNew) saveAndNew() }} placeholder="Ex: Trousse de secours…" style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;margin-bottom:14px;')} />
        {onSubmitAndNew && (

...
export function ChangelogModal({ isOpen, onClose, sx, currentBuild }) {
  ...
  return (
    <ModalShell onClose={onClose} z={210} fade={true}>
      <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()} style={sx('width:100%;background:#f6efe2;border-radius:28px 28px 0 0;padding:20px 20px 36px;max-height:84vh;overflow-y:auto;animation:sheetUp 0.3s cubic-bezier(0.2,0.8,0.2,1);')}>
        <div style={sx('width:40px;height:4px;border-radius:4px;background:#d8cbb0;margin:0 auto 16px;')} />
        <div style={sx('font-family:Quicksand;font-weight:700;font-size:22px;margin-bottom:2px;')}>🗂️ Historique des versions</div>

  ...
  return (
    <ModalShell onClose={onClose} z={200} fade={false}>
      <div role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={sx('width:100%;background:#f6efe2;border-radius:28px 28px 0 0;padding:20px;max-height:80vh;overflow-y:auto;animation:sheetUp 0.3s;')}>
        ...
        <div style={sx('font-family:Quicksand;font-weight:700;font-size:19px;margin-bottom:12px;')}>Éditer jour</div>
        <div style={sx('margin-bottom:12px;')}>
          ...
          <input id="edit-day-title" type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Ex: Arrivée à Vezels" style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')} />
        </div>
        <div style={sx('margin-bottom:14px;')}>
          ...
          <input id="edit-day-sub" type="text" value={editSub} onChange={e => setEditSub(e.target.value)} placeholder="Ex: Installation au gîte" style={sx('width:100%;border:1px solid #d8cbb0;background:#fffdf8;border-radius:12px;padding:12px 14px;font-size:15px;')} />
        </div>

  ...
  return (
    <div data-testid="screen-souvenirs" style={sx('padding:16px 18px 40px;')}>
      <div style={sx('background:#9c6b4a;border-radius:20px;padding:18px;color:#fffaf0;box-shadow:0 8px 20px rgba(156,107,74,0.2);')}>
        ...
        <div style={sx('font-size:13px;color:#f1e4d4;margin-top:4px;')}>{photos.length ? `${photos.length} photo${photos.length > 1 ? 's' : ''}, rangée${photos.length > 1 ? 's' : ''} par journée` : 'Les photos prises ici se rangent toutes seules par journée.'}</div>
      </div>

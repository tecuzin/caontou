  ...
  return (
    <div data-testid="screen-visites">
      <div style={sx('padding:54px 18px 4px;display:flex;align-items:flex-start;justify-content:space-between;gap:10px;')}>
        ...
        {openVote && <button data-testid="btn-open-vote" onClick={openVote} style={sx('flex:0 0 auto;margin-top:4px;border:none;background:#4a5d3a;color:#fffaf0;font-weight:700;font-family:Quicksand;font-size:13px;border-radius:12px;padding:10px 12px;cursor:pointer;')}>🗳️ Voter</button>}
      </div>
      <div style={sx('display:flex;gap:8px;overflow-x:auto;padding:12px 18px 14px;')}>
        {FILTERS.map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={sx(`flex:0 0 auto;border:1px solid ${filter === f ? '#4a5d3a' : '#ece2cf'};background:${filter === f ? '#4a5d3a' : '#fffdf8'};color:${filter === f ? '#fffaf0' : '#6b6354'};border-radius:999px;padding:8px 16px;font-weight:700;font-size:13px;cursor:pointer;`)}>{f}</button>
        ))}
      </div>
      <div style={sx('display:flex;gap:8px;padding:0 18px 14px;')}>
        {[['dist', '📍 Distance'], ['cat', '🏷️ Catégorie']].map(([k, label]) => (
          <button key={k} onClick={() => setVisitSort(visitSort === k ? null : k)} style={sx(`flex:0 0 auto;border:1px solid ${visitSort === k ? '#4a5d3a' : '#ece2cf'};background:${visitSort === k ? '#4a5d3a' : '#fffdf8'};color:${visitSort === k ? '#fffaf0' : '#6b6354'};border-radius:999px;padding:6px 12px;font-weight:700;font-size:12px;cursor:pointer;`)}>{label}</button>
        ))}
      </div>
      <div style={sx('padding:0 18px 8px;display:flex;justify-content:flex-end;')}>
        <button onClick={() => { setEditingVisitId(null); setNewVisitName(''); setNewVisitDist(''); setNewVisitDur(''); setNewVisitAge(''); setNewVisitCat('Nature'); setShowVisitEdit(true) }} style={sx('border:1.5px dashed #c2a778;background:#fbf4e6;color:#9c6b4a;font-weight:700;font-family:Quicksand;font-size:13px;border-radius:12px;padding:6px 14px;cursor:pointer;')}>+ Ajouter visite</button>
      </div>
            ...
            <div key={v.id} style={sx('background:#fffdf8;border:1px solid #efe6d4;border-radius:16px;padding:12px;box-shadow:0 2px 8px rgba(74,93,58,0.05);')}>
              ...
              {rateVisit && (
                <div data-testid={`visit-rating-${v.id}`} style={sx('display:flex;align-items:center;gap:4px;margin-top:10px;padding-top:10px;border-top:1px solid #f1e9da;')}>
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button key={n} aria-label={`Noter ${v.name} ${n} sur 5`} onClick={() => rateVisit(v.id, n === stars ? 0 : n)} style={sx(`border:none;background:transparent;cursor:pointer;font-size:19px;line-height:1;padding:0 2px;color:${n <= stars ? '#cf7d3c' : '#d8cbb0'};`)}>★</button>
                  ))}

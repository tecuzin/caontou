...
import { KidsLockBar } from './components/KidsLockBar.jsx'
      ...
      {sub && (
        <div data-testid="sub-screen-wrapper" onTouchStart={subScreenSwipe.onTouchStart} onTouchEnd={subScreenSwipe.onTouchEnd} style={sx('height:100%;display:flex;flex-direction:column;')}>
          ...
          <div key={sub} className="screen-in" style={sx('flex:1;overflow-y:auto;')}>
            ...
            {/* SOUVENIRS */}
            {sub === 'souvenirs' && (
              <Souvenirs sx={sx} photos={photos} days={days} srcMap={srcMap} capturePhoto={capturePhoto} pickPhotos={pickPhotos} deletePhoto={deletePhoto} loadSrc={loadSrc} shareDay={shareDay} journal={journal} openDayJournal={openDayJournal} trip={trip} />
            )}

            {/* BINGO */}
            {sub === 'bingo' && (
              <Bingo sx={sx} items={bingoItems} checked={bingo} toggleBingo={toggleBingo}
                currentChild={currentChild} setCurrentChild={setCurrentChild} childOptions={childOptions} />
            )}
            ...
            {/* LEXIQUE AUVERGNAT */}
            {sub === 'lexique' && (
              <Dialect sx={sx} words={dialectWords} setWords={setDialectWords} />
            )}

            {/* COIN DESSIN */}
            {sub === 'dessin' && (
              <DrawPad sx={sx} onSave={saveDrawing} />
            )}

            {/* TOISE */}
            {sub === 'toise' && (
              <Heights sx={sx} heights={heights} onAdd={addHeightEntry} onRemove={removeHeightEntry} />
            )}
            ...
            {sub === 'reglages' && (
              <Reglages sx={sx} isOn={isOn} toggleFeature={toggleFeature} relaunchOnboarding={() => { setOnboarded(false); setSub(null) }}
                ...
                sunMode={sunMode} setSunMode={setSunMode}
                kidsLock={kidsLock} lockKids={lockKids} storage={storage}
